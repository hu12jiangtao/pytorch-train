...
if __name__ == '__main__':
    ...
    dataset = datasets.cifar.CIFAR10(root='D:\\python\\pytorch作业\\all_data\\cifar10\\data', train=False, transform=test_transform,
                                     download=True)
    # dataset = datasets.cifar.CIFAR10(root='data', train=False, transform=test_transform,download=True)
    features = []
    labels = []
    model_q = get_model()
    ...
    else: # 如何将一张图片转换为一个特征(利用的是encode_q将一张图片转换为一个向量)
        with torch.no_grad():
    ...
    tensor_candidate_sample = torch.tensor(features).to(config.DEVICE).transpose(0, 1) # [128,batch]
