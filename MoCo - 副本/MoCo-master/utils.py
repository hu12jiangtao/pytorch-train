...
def get_shuffle_idx(bs, device):  # 输入的是批量的大小
    """shuffle index for ShuffleBN """
    # 变成一个一维向量(相当于shuffle(torch.arrange(bs))), 其中元素[0, bs)
    ...
    # 将arrange_index中的值按照shuffle_value的索引顺序赋值到reverse_idx中去
    # 此时执行的操作类似于np.argsort操作，获得从小到大的值在shuffle_value的索引的位置
...
def get_transform(image_size, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], mode='train', to_tensor=True):
    # transform的数据集增强是针对于PIL的图片的(不是PIL的通道数放在最后的矩阵)
    transform_to_tensor = [
        transforms.ToTensor(),
