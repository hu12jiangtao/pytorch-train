...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
        ...
        self.conv2 = nn.Conv2d(out_planes, out_planes, kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.droprate = dropRate # 正则化dropout的值
        self.equalInOut = (in_planes == out_planes) # 用来判断这个模块的输入通道是否和输出通道相同
...
class WideResNet(nn.Module):
    ...
    def features(self, x): # 到达最后的输出层的模型结构
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
